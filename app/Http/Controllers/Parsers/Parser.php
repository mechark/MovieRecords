...
class Parser
{
    ...
    public function parserMovie(string $url)
    {
...
//                print_r(curl_getinfo($ch, CURLINFO_HEADER_OUT));
    ...
    {
        $moviePages = $this->getHtml('https://kinogo.la/film/boeviki/');
        $dom = new Dom;
...
//        foreach ($moviePages as $movieUrl) {
        ...
        $nameOfTheMovie = array_diff($nameOfTheMovie, array(''));
        array_pop($nameOfTheMovie);
        $nameOfTheMovie = implode("+", $nameOfTheMovie);
        $yearOfTheMovie = $page->find('.quote a')[0]->plaintext;
        $kinogoSearch = "https://www.kinopoisk.ru/index.php?kp_query=" . $nameOfTheMovie;
        $dom->setOptions(
            (new Options())->setRemoveScripts(false)
        );
        // Непосредственно получение страницы
        $dom->loadFromUrl($kinogoSearch);
        $retrievalName = 'https://www.kinopoisk.ru/' . $dom->find('.name a')[0]->href;
        $retrievalPage = $this->getCurl($retrievalName);

            // Распарсинг
            $dom->loadStr($retrievalPage);
            ...
            if ($page->aggregateRating->ratingValue != null)
            {
